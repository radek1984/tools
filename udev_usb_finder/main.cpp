...
void find(uint16_t vid, uint16_t pid, const char *dev_class,
  ...
  struct udev_list_entry *usb_dev_ifce_list =
  ...
  udev_list_entry_foreach(ifce_it, usb_dev_ifce_list) {
    ...
    bool vid_ok = false;
    // ifce:
    struct udev_device *ifce_dev = udev_device_new_from_syspath(
        udev_ctx, udev_list_entry_get_name(ifce_it));

    if (ifce_dev != nullptr) {
      // the usb device:
      struct udev_device *device =
          udev_device_get_parent_with_subsystem_devtype(ifce_dev, "usb",
                                                        "usb_device");
      if (device != nullptr) {
...
int main() {
  ...
  for (auto &n : out_dev_ifce_nodes) std::cout << n << std::endl;
  std::cout << std::endl;
